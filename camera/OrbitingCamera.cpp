...
void OrbitingCamera::mousePan(int x, int y) {
    auto dx = -(x - m_oldX) * (0.001 * m_zoomZ);
    auto dy = (y - m_oldY) * (0.001 * m_zoomZ);
    auto dt = glm::inverse(getViewMatrix()) * glm::vec4{dx, dy, 0, 0};
