...
FBO::FBO(int numberOfColorAttachments, DEPTH_STENCIL_ATTACHMENT attachmentType, int width, int height,
    ...
    m_handle(0),
    m_isDepthMap(attachmentType == DEPTH_STENCIL_ATTACHMENT::DEPTH_MAP ? true : false),
    m_width(width),
    m_height(height)
{
    ...
    if (numberOfColorAttachments && attachmentType != DEPTH_STENCIL_ATTACHMENT::NONE)
...
void FBO::generateDepthStencilAttachment() {
    switch(m_depthStencilAttachmentType) {
        case DEPTH_STENCIL_ATTACHMENT::DEPTH_ONLY:
            ...
            // TODO [Task 8]
            break;
        case DEPTH_STENCIL_ATTACHMENT::DEPTH_MAP:
            m_depthMap = std::make_unique<Texture2D>(nullptr, m_width, m_height, GL_FLOAT, GL_DEPTH_COMPONENT);
            m_depthMap->bind();
...
const Texture2D& FBO::getDepthMapAttachment() const {
