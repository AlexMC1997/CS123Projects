...
bool CS123XmlSceneParser::parsePrimitive(const QDomElement &prim, CS123SceneNode* node) {
    ...
    while (!childNode.isNull()) {
        ...
        } else if (e.tagName() == "metalness") {
            if (!parseSingle(e, mat.metalness, "v")) {
                PARSE_ERROR(e);
                return false;
            }
        } else if (e.tagName() == "glossiness") {
            if (!parseSingle(e, mat.glossiness, "v")) {
                PARSE_ERROR(e);
                return false;
            }
        } else if (e.tagName() == "ior") {
            if (!parseTriple(e, mat.ior.r, mat.ior.g, mat.ior.b, "r", "g", "b")) {
                PARSE_ERROR(e);
                return false;
            }
        } else if (e.tagName() == "abs") {
            if (!parseTriple(e, mat.abs.r, mat.abs.g, mat.abs.b, "r", "g", "b")) {
                PARSE_ERROR(e);
                return false;
            }
